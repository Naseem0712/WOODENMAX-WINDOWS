...
export function getSlidingCuttingPlanPerWindow(
  ...
  // 2) Bottom track clips only: one clip per track lane (2T→2 pc, 3T→3 pc), width each; separate clip powder rate.
  ...
  addLine(lines, `Track Clip (${trackCount} pc, bottom)`, trackCount, widthFt, clipRate, 0);
