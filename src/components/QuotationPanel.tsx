...
interface QuotationPanelProps {
    ...
    /** Per-window special note (saved with design / quotation). */
    windowNotes?: string;
    setWindowNotes?: (value: string) => void;
}
        ...
        <div className="overflow-x-hidden border-t-2 border-slate-700 bg-slate-800 p-1.5 shadow-inner sm:p-2 lg:rounded-none lg:p-2 rounded-t-lg">
            ...
            {setWindowNotes ? (
              <div className="mx-auto mt-1.5 max-w-7xl">
                <label htmlFor={`${idPrefix}quote-window-notes`} className="mb-0.5 block text-[10px] font-medium text-slate-400">
                  Special note (this window)
                </label>
                <textarea
                  id={`${idPrefix}quote-window-notes`}
                  name="quote-window-notes"
                  rows={2}
                  placeholder="Prints with this window on the PDF…"
                  value={windowNotes ?? ''}
                  onChange={(e) => setWindowNotes(e.target.value)}
                  className="w-full rounded-md border border-slate-600 bg-slate-900 px-2 py-1.5 text-xs text-slate-100 placeholder:text-slate-500 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                />
              </div>
            ) : null}
