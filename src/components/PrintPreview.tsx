...
export const PrintPreview: React.FC<PrintPreviewProps> = ({ isOpen, onClose, items, settings, setSettings }) => {
  ...
  const handleExportPdf = () => {
    ...
    if (!element || isExporting || isPrinting) {
        return;
    }
    ...
    try {
        ...
        filename: quotationPdfFilename(customerName, pdfDateStamp),
        image: { type: 'jpeg' as const, quality: 0.95 },
        html2canvas: {
            scale: window.matchMedia('(max-width: 768px)').matches ? 1.5 : 2,
            logging: false,
            useCORS: true,
            allowTaint: true,
    ...
    import('html2pdf.js')
        .then((mod: { default?: unknown }) => {
            // html2pdf.js ships without typings; runtime shape is a callable worker factory.
